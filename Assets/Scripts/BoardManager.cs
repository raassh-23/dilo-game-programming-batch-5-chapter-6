...
public class BoardManager : MonoBehaviour
{
    ...
    private void Start()
    {
        Vector2 tileSizes = tilePrefab.GetComponent<SpriteRenderer>().size;
        CreateBoard(tileSizes);
        ...
        for (int x = 0; x < size.x; x++)
        {
            for (int y = 0; y < size.y; y++)
            {
                TileController newTile = Instantiate(tilePrefab,
                                            GetIndexPosition(new Vector2Int(x, y)),
                                            tilePrefab.transform.rotation,
                                            transform).GetComponent<TileController>();

                List<int> possibleId = GetStartingPossibleId(x, y);
    ...
    private List<int> GetStartingPossibleId(int x, int y)
    {
    ...
    private void ProcessMatches()
    {
        ...
        combo++;
        ScoreManager.Instance.IncrementCurrectScore(matchingTiles.Count, combo);
